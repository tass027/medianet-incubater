...
/*
  DESIGN PHILOSOPHY
  ─────────────────
  Refined editorial minimalism. Google Forms DNA reimagined for a
  professional admin context.

  Typography: "Plus Jakarta Sans" — clean, confident, modern.
  Palette: Warm-neutral surfaces, ink text, vivid accent system.
  Motion: Subtle entrance animations + fluid micro-interactions.
  Layout: True 3-column builder mirroring Google Forms proportions.
  Dark: Deep charcoal surfaces — not pure black, not grey soup.
*/
...
const tk = (dark, accent) => {
  ...
  return {
    ...
    // shadows
    shadow    : L
      ? '0 1px 2px rgba(20,15,40,.04), 0 4px 16px rgba(20,15,40,.07)'
      : '0 1px 2px rgba(0,0,0,.3), 0 4px 16px rgba(0,0,0,.4)',
    shadowLg  : L
      ? '0 8px 24px rgba(20,15,40,.10), 0 2px 6px rgba(20,15,40,.06)'
      : '0 8px 24px rgba(0,0,0,.5), 0 2px 6px rgba(0,0,0,.3)',
...
const IC = {
  ...
  chart  : <Svg d={<><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></>}/>,
  spin   : (
...
const Divider = ({T}) => (
  <div style={{height:1, background:T.line, margin:'0 -1px'}} />
);
...
export default function NewFormPage() {
  ...
  // Dark mode — syncs with DashboardLayout
  ...
  const [questions, setQuestions] = useState([mkQ('radio')]);
  const [activeId,  setActiveId]  = useState(questions[0].id);
  const [recips,    setRecips]    = useState([]);
  const [srch,      setSrch]      = useState('');
  const [saving,    setSaving]    = useState(false);
  const [publishing,setPub]       = useState(false);
  const [published, setPubd]      = useState(false);
  ...
  const onDraft = () => {
    setSaving(true);
    setTimeout(()=>{ setSaving(false); toast$('Draft saved'); setTimeout(()=>router.push('/dashboard/admin/forms'),700); },800);
  };
  const onPublish = () => {
    if(!title)        return toast$('Please add a form title','warn');
    if(!recips.length) return toast$('Select at least one recipient','warn');
    setPub(true);
    setTimeout(()=>{ setPub(false); setPubd(true); toast$('Form published successfully!'); setTimeout(()=>router.push('/dashboard/admin/forms'),1400); },1100);
  };
  ...
  const TABS = [
    ...
    {id:'responses', label:'Responses', icon:IC.chart },
        ...
        <div style={{fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif", display:'flex', flexDirection:'column', gap:16, paddingBottom:56}}>
          ...
          {toast && (
            <div style={{
              ...
              <span style={{display:'flex', color: toast.type==='warn' ? T.warnText : toast.type==='err' ? T.errText : '#22C55E'}}>
          ...
          <div className="_up" style={{
            ...
            <button onClick={()=>router.push('/dashboard/admin/forms')}
              style={{
                ...
                padding:'7px 14px 7px 8px', borderRadius:10,
                border:'none', background:'none', cursor:'pointer',
                color:T.ink3, fontSize:13, fontWeight:600,
                fontFamily:'inherit', transition:'all .15s', marginRight:8,
              }}
            ...
            <div style={{display:'flex', alignItems:'center', gap:10, flex:1, minWidth:0}}>
              ...
              <span style={{
                ...
                {title || <span style={{color:T.ink3, fontWeight:400}}>Untitled form</span>}
              </span>
              {/* Progress pill */}
              <div style={{
                display:'flex', alignItems:'center', gap:6,
                padding:'4px 10px', borderRadius:99,
                background: pct===100 ? T.okBg : T.acBg,
                border:`1px solid ${pct===100 ? T.okText+'30' : T.acLine}`,
                marginLeft:4,
              }}>
                <div style={{
                  width:32, height:3, borderRadius:99, background:T.line2, overflow:'hidden',
                }}>
            ...
            <div style={{
              display:'flex', gap:1, padding:'4px',
              background:T.card2, borderRadius:10, border:`1px solid ${T.line}`,
            }}>
              {TABS.map(t=>(
            ...
            <div style={{display:'flex', alignItems:'center', gap:8}}>
              <button onClick={onDraft} disabled={saving}
                style={{
                  display:'flex', alignItems:'center', gap:6,
                  padding:'8px 14px', borderRadius:9, border:`1px solid ${T.line}`,
              ...
              {published ? (
                <div style={{
                  ...
                  background:T.okBg, color:T.okText,
                  fontSize:13, fontWeight:700,
                  border:`1px solid ${T.okText}30`,
                }}>
                  {IC.check}<span>Published</span>
                </div>
              ) : (
                <button onClick={onPublish} disabled={publishing}
                  style={{
                    ...
                    boxShadow:`0 2px 12px rgba(${T.acRgb},.32)`,
                    opacity:publishing?.55:1, transition:'all .18s',
                  }}
                  onMouseEnter={e=>{if(!publishing){e.currentTarget.style.transform='translateY(-1px)'; e.currentTarget.style.boxShadow=`0 4px 20px rgba(${T.acRgb},.44)`;} }}
                  onMouseLeave={e=>{e.currentTarget.style.transform='none'; e.currentTarget.style.boxShadow=`0 2px 12px rgba(${T.acRgb},.32)`;}}>
                  {publishing?IC.spin:IC.send}
          ...
          {tab==='build' && (
            <div className="_up _d1" style={{
              ...
              <div style={{display:'flex', flexDirection:'column', gap:12}}>
                ...
                <div style={{...card, overflow:'hidden'}}>
                  <div style={{padding:'14px 16px 10px', display:'flex', alignItems:'center', justifyContent:'space-between'}}>
                    <span style={{fontSize:11, fontWeight:700, textTransform:'uppercase', letterSpacing:'.08em', color:T.ink3}}>
                      Questions
                    </span>
                    <span style={{
                      fontSize:11, fontWeight:700,
                      padding:'2px 8px', borderRadius:99,
                      background:T.acBg, color:T.acText,
                    }}>{questions.length}</span>
                  </div>
                  <Divider T={T}/>
                  <div style={{padding:'8px 8px', maxHeight:220, overflowY:'auto'}} className="_scroll">
                    {questions.map((q,i)=>(
                      <div key={q.id}
                        draggable
                        ...
                        style={{
                          ...
                          background:activeId===q.id ? T.active : 'transparent',
                        ...
                        <span style={{
                          ...
                          background:activeId===q.id ? accent : T.line,
                          color:activeId===q.id ? '#fff' : T.ink3,
                          transition:'all .16s',
                        }}>{i+1}</span>
                        <span style={{
                          fontSize:12, fontWeight:activeId===q.id?600:400,
                          color:activeId===q.id?T.ink:T.ink2,
                          ...
                          {QTYPES.filter(t=>t.group===g).map(t=>(
                            <button key={t.id} onClick={()=>addQ(t.id)}
                              style={{
                                ...
                                color:T.ink2, transition:'all .14s', textAlign:'left',
                              }}
                    ...
                    <>
                      ...
                      <div style={{padding:'12px 16px'}}>
                        <div style={{display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:8}}>
                          {PALETTE.map(c=>(
                            <button key={c.hex} onClick={()=>{setAccent(c.hex);}}
                              title={c.label}
                              style={{
                                height:32, borderRadius:8, border:'none',
                                background:c.hex, cursor:'pointer',
                                position:'relative',
                                boxShadow:accent===c.hex ? `0 0 0 2px ${T.card}, 0 0 0 4px ${c.hex}` : 'none',
                                transform:accent===c.hex ? 'scale(1.08)' : 'scale(1)',
                                transition:'all .15s',
                              }}>
                              {accent===c.hex && (
                                <span style={{
              ...
              <div style={{display:'flex', flexDirection:'column', gap:12, minWidth:0}}>
                  ...
                  <div style={{padding:'24px 28px 22px'}}>
                    <textarea rows={1} value={title} onChange={e=>setTitle(e.target.value)}
                      ...
                      style={{
                        ...
                        borderBottom:`2.5px solid ${title?accent:T.line2}`,
                        paddingBottom:8, marginBottom:14, transition:'border-color .18s',
                        letterSpacing:'-.02em',
                      }}
                ...
                <button onClick={()=>addQ('short')}
                  style={{
                    ...
                    borderRadius:14, border:`2px dashed ${T.line2}`,
                    background:'transparent', color:T.ink3,
              ...
              <div style={{display:'flex', flexDirection:'column', gap:12}}>
                ...
                <div style={{...card, overflow:'hidden'}}>
                  <div style={{padding:'14px 16px 12px', display:'flex', alignItems:'center', justifyContent:'space-between'}}>
                    ...
                    <span style={{
                      ...
                      background:recips.length ? accent : T.line,
                      color:recips.length ? '#fff' : T.ink3,
                      transition:'all .2s',
                    }}>{recips.length}/{MOCK_APPS.length}</span>
                  </div>
                  <Divider T={T}/>
                  ...
                  <div style={{padding:'10px 12px'}}>
                    ...
                    <button onClick={()=>setRecips(recips.length===MOCK_APPS.length?[]:MOCK_APPS.map(a=>a.id))}
                      style={{
                        ...
                        border:`1px dashed ${T.acLine}`,
                        background:recips.length===MOCK_APPS.length ? T.acBg2 : 'transparent',
                        color:T.acText, fontSize:11, fontWeight:700,
                        ...
                        return (
                          <div key={app.id} onClick={()=>toggleR(app.id)}
                            style={{
                              ...
                              border:`1px solid ${on ? T.acLine : T.line}`,
                              background:on ? T.acBg : T.card,
                              transition:'all .16s',
                            }}
                            ...
                            <div style={{
                              ...
                              background:`linear-gradient(135deg,${app.color}ee,${app.color}88)`,
                              display:'flex', alignItems:'center', justifyContent:'center',
                              color:'#fff', fontWeight:800, fontSize:11, letterSpacing:'-.02em',
                            }}>{app.name.slice(0,2).toUpperCase()}</div>
                            ...
                            <div style={{display:'flex', flexDirection:'column', alignItems:'flex-end', gap:4, flexShrink:0}}>
                              <Chip color={app.color} T={T}>{app.sector}</Chip>
                              <div style={{
                                width:15, height:15, borderRadius:'50%',
                                border:`2px solid ${on?accent:T.line2}`,
                                background:on?accent:'transparent',
                                display:'flex', alignItems:'center', justifyContent:'center',
                                color:'#fff', fontSize:8, fontWeight:800, transition:'all .16s',
                              }}>{on&&'✓'}</div>
                            </div>
                          </div>
                        );
                    ...
                    {!recips.length && (
                      <div style={{
                        ...
                        background:T.warnBg, border:`1px solid ${T.warnText}30`,
                        display:'flex', alignItems:'center', gap:8,
                      }}>
                        <span style={{color:T.warnText, display:'flex', flexShrink:0}}>{IC.warn}</span>
                        <p style={{margin:0, fontSize:11.5, color:T.warnText, fontWeight:600}}>Select at least one recipient to publish</p>
                      </div>
                    )}
                  </div>
                </div>
                ...
                {/* Summary card */}
                <div style={{
                  borderRadius:16, background:accent, position:'relative', overflow:'hidden',
                  boxShadow:`0 6px 24px rgba(${T.acRgb},.28)`,
                }}>
                  <div style={{
                    position:'absolute', inset:0, pointerEvents:'none',
                    backgroundImage:`radial-gradient(circle at 80% 20%, rgba(255,255,255,.12) 0%, transparent 60%)`,
                  }}/>
          ...
          {tab==='responses' && <ResponsesSection T={T} accent={accent} toast$={toast$}/>}
...
function QCard({q,idx,isActive,accent,T,onActivate,onChange,onDup,onDel,canDel,addOpt,delOpt,editOpt,addRow,delRow,editRow}) {
  ...
  return (
    <div onClick={onActivate} style={{
      ...
      border:`1.5px solid ${isActive ? accent : T.line}`,
      boxShadow:isActive ? `${T.shadowLg}, ${T.ring}` : T.shadow,
      ...
      <div style={{
        ...
        background:accent, opacity:isActive?1:0,
        transition:'opacity .2s', borderRadius:'15px 0 0 15px',
      }}/>
      ...
      <div style={{
        ...
        background:isActive ? T.acBg : T.card2,
        borderBottom:`1px solid ${isActive ? T.acLine : T.line}`,
        transition:'all .18s',
      }}>
        <div style={{display:'flex', alignItems:'center', gap:8}}>
          <span style={{
            ...
            background:isActive?accent:T.line,
        ...
        {isActive && (
          <div style={{display:'flex', alignItems:'center', gap:2}} onClick={e=>e.stopPropagation()}>
            {[
              {fn:onDup, ic:IC.copy,  ok:false},
              ...(canDel?[{fn:onDel,ic:IC.trash,ok:true}]:[]),
            ].map(({fn,ic,ok},i)=>(
              <button key={i} onClick={fn}
                style={{
                  width:30, height:30, borderRadius:8,
                  border:'none', display:'flex', alignItems:'center', justifyContent:'center',
                  cursor:'pointer', background:'transparent', color:T.ink3,
                  transition:'all .14s',
                }}
      ...
      <div style={{padding:'18px 20px 16px 20px', display:'flex', flexDirection:'column', gap:14}}>
        ...
        <div style={{display:'flex', gap:12, alignItems:'flex-start'}}>
          <textarea rows={1} value={q.title} onChange={e=>onChange({title:e.target.value})}
            ...
            style={{
              ...
              borderBottom:`2px solid ${T.line2}`, paddingBottom:6, transition:'border-color .18s',
            }}
            onInput={e=>{e.target.style.height='auto'; e.target.style.height=e.target.scrollHeight+'px';}}
            onFocus={e=>e.target.style.borderBottomColor=accent}
            onBlur={e=>e.target.style.borderBottomColor=T.line2}
          ...
          {isActive && (
            <select value={q.type}
              ...
              onChange={e=>{
                ...
                onChange({type:t,
                  options:['radio','checkbox','dropdown','grid'].includes(t)?(q.options.length?q.options:['Option 1','Option 2']):[],
                  rows:t==='grid'?(q.rows.length?q.rows:['Row 1']):[],
                });
              }}
              style={{
        ...
        {isActive && (
          <input value={q.description} onChange={e=>onChange({description:e.target.value})}
            ...
            style={{
              ...ib, fontSize:13, color:T.ink3,
              borderBottom:`1px dashed ${T.line2}`, paddingBottom:5, transition:'border-color .18s',
            }}
            onFocus={e=>e.target.style.borderBottomColor=accent}
            onBlur={e=>e.target.style.borderBottomColor=T.line2}
          />
        ...
        {/* Short / Long placeholder */}
        {q.type==='short' && <div style={{fontSize:13, color:T.ink3, borderBottom:`1px solid ${T.line2}`, paddingBottom:7, transition:'border-color .18s'}}>Short answer text</div>}
          ...
          <div style={{display:'flex', flexDirection:'column', gap:8}}>
            {q.options.map((opt,i)=>(
              <div key={i} style={{display:'flex', alignItems:'center', gap:10}}>
                <span style={{
                  display:'flex', alignItems:'center', justifyContent:'center',
                  flexShrink:0, color:T.ink3, fontSize:15,
                  width:20, textAlign:'center', lineHeight:1,
                }}>
                  {q.type==='radio'&&'○'}
                  {q.type==='checkbox'&&'□'}
                  {q.type==='dropdown'&&<span style={{fontSize:11, fontWeight:700}}>{i+1}.</span>}
                </span>
                <input value={opt} onChange={e=>editOpt(i,e.target.value)}
                  onClick={e=>e.stopPropagation()}
                  placeholder={`Option ${i+1}`}
                  style={{
                    ...ib, flex:1, fontSize:13.5,
                    borderBottom:`1px solid ${T.line2}`, paddingBottom:5,
                    transition:'border-color .16s',
                  }}
                  onFocus={e=>e.target.style.borderBottomColor=accent}
                  onBlur={e=>e.target.style.borderBottomColor=T.line2}
                ...
                {isActive && q.options.length>1 && (
                  <button onClick={e=>{e.stopPropagation();delOpt(i);}}
                    style={{
            ...
            {isActive && (
              <button onClick={e=>{e.stopPropagation();addOpt();}}
                style={{
                  alignSelf:'flex-start', border:'none', background:'none',
                  cursor:'pointer', fontSize:12.5, fontWeight:700,
                  fontFamily:'inherit', color:accent, padding:'2px 0',
        ...
        {hasRows && (
          <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:20}}>
            {[
              {label:'Rows',    items:q.rows,    edit:editRow, del:delRow, add:addRow,  sym:'›'},
              {label:'Columns', items:q.options, edit:editOpt, del:delOpt, add:addOpt,  sym:'◉'},
            ].map(col=>(
              <div key={col.label}>
                ...
                {col.items.map((it,i)=>(
                  <div key={i} style={{display:'flex', alignItems:'center', gap:8, marginBottom:7}}>
                    ...
                    <input value={it} onChange={e=>col.edit(i,e.target.value)}
                      onClick={e=>e.stopPropagation()}
                    ...
                    {isActive && col.items.length>1 && (
                      <button onClick={e=>{e.stopPropagation();col.del(i);}}
                        style={{width:24, height:24, borderRadius:5, border:'none', background:'transparent', color:T.ink3, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, transition:'all .13s'}}
                        onMouseEnter={e=>{e.currentTarget.style.background=T.errBg; e.currentTarget.style.color=T.errText;}}
                        onMouseLeave={e=>{e.currentTarget.style.background='transparent'; e.currentTarget.style.color=T.ink3;}}>
                        {IC.x}
                      </button>
                ...
                {isActive && (
                  <button onClick={e=>{e.stopPropagation();col.add();}}
                    style={{border:'none', background:'none', cursor:'pointer', fontSize:11.5, fontWeight:700, fontFamily:'inherit', color:accent, padding:'2px 0'}}>
                    ＋ Add
                  </button>
                )}
              </div>
                ...
                <div key={f.k} style={{display:'flex', alignItems:'center', gap:7}}>
                  ...
                  <input type="number" min={f.mn} max={f.mx} value={f.v}
                    ...
                    style={{
                      ...
                      padding:'6px', fontSize:13, outline:'none',
                      fontFamily:'inherit', background:T.card2, color:T.ink,
                      transition:'border-color .15s',
                    }}
                    onFocus={e=>e.target.style.borderColor=accent}
                    onBlur={e=>e.target.style.borderColor=T.line}
            ...
            <div style={{display:'flex', gap:6, flexWrap:'wrap', padding:'10px 12px', background:T.card2, borderRadius:10}}>
              {Array.from({length:q.scaleMax-q.scaleMin+1},(_,i)=>i+q.scaleMin).map(n=>(
                <div key={n} style={{
                  ...
                  border:`1.5px solid ${T.acLine}`,
                  display:'flex', alignItems:'center', justifyContent:'center',
                  fontSize:12, fontWeight:700, color:T.acText,
                }}>{n}</div>
              ))}
            </div>
            <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:10}}>
              {[
                {k:'scaleMinLabel', ph:`Label ${q.scaleMin}`, v:q.scaleMinLabel, al:'left'},
                {k:'scaleMaxLabel', ph:`Label ${q.scaleMax}`, v:q.scaleMaxLabel, al:'right'},
              ].map(f=>(
                <input key={f.k} value={f.v}
                  onChange={e=>onChange({[f.k]:e.target.value})}
                  onClick={e=>e.stopPropagation()}
                  placeholder={f.ph}
                  style={{
                    ...{fontFamily:"'Plus Jakarta Sans',sans-serif", color:T.ink3, background:'transparent', border:'none', outline:'none', width:'100%'},
                    fontSize:12, borderBottom:`1px solid ${T.line2}`, paddingBottom:4,
                    textAlign:f.al, transition:'border-color .15s',
                  }}
                  onFocus={e=>e.target.style.borderBottomColor=accent}
                  onBlur={e=>e.target.style.borderBottomColor=T.line2}
                />
        ...
        {q.type==='date' && (
          <input type="date" disabled style={{border:`1px solid ${T.line}`, borderRadius:9, padding:'8px 12px', fontSize:13, color:T.ink3, background:T.card2, fontFamily:'inherit', cursor:'not-allowed', opacity:.5}}/>
        )}
        {q.type==='time' && (
          <input type="time" disabled style={{border:`1px solid ${T.line}`, borderRadius:9, padding:'8px 12px', fontSize:13, color:T.ink3, background:T.card2, fontFamily:'inherit', cursor:'not-allowed', opacity:.5}}/>
        )}
        {q.type==='file' && (
          <div style={{
            border:`1.5px dashed ${T.acLine}`, borderRadius:10, padding:'16px',
            textAlign:'center', color:T.ink3, fontSize:12.5, background:T.acBg,
          ...
          <div style={{
            ...
            <button
              ...
              style={{
                ...
                background:q.required?accent:T.line2,
                transition:'background .2s',
                boxShadow:q.required?`0 0 0 3px rgba(${T.acRgb},.18)`:'none',
              }}>
              <div style={{
                position:'absolute', top:2, left:q.required?20:2,
...
function PreviewSection({title,subtitle,accent,questions,T}) {
  ...
  if (done) return (
    <div style={{display:'flex', flexDirection:'column', alignItems:'center', padding:'80px 24px', gap:16, textAlign:'center'}}>
      <div style={{
        width:68, height:68, borderRadius:'50%', background:`linear-gradient(135deg,${accent},${accent}99)`,
        display:'flex', alignItems:'center', justifyContent:'center', color:'#fff', fontSize:28,
        boxShadow:`0 8px 24px rgba(${rgb},.35)`, animation:'_pop .4s cubic-bezier(.22,1,.36,1)',
      }}>✓</div>
      ...
      <button onClick={()=>{setDone(false);setRes({});}}
        style={{
          marginTop:8, padding:'10px 28px', background:accent, color:'#fff',
          border:'none', borderRadius:100, fontSize:13.5, fontWeight:700,
          cursor:'pointer', fontFamily:'inherit',
          boxShadow:`0 2px 12px rgba(${rgb},.35)`,
        }}>
    ...
    <div style={{maxWidth:660, margin:'0 auto', width:'100%', paddingBottom:48}}>
      ...
      {questions.map(q=>(
        <div key={q.id} style={{background:T.card, border:`1px solid ${T.line}`, borderRadius:16, boxShadow:T.shadow, padding:'20px 26px', marginBottom:10}}>
          <p style={{margin:'0 0 4px', fontSize:14.5, fontWeight:600, color:T.ink}}>
            {q.title||'Untitled question'}
            {q.required && <span style={{color:T.errText, marginLeft:4}}>*</span>}
          </p>
      ...
      <button onClick={()=>setDone(true)}
        style={{
          padding:'12px 34px', background:accent, color:'#fff',
          border:'none', borderRadius:100, fontSize:14, fontWeight:700,
          cursor:'pointer', fontFamily:'inherit',
          boxShadow:`0 2px 14px rgba(${rgb},.32)`, transition:'all .18s',
        }}
...
//  RESPONSES SECTION
// ════════════════════════════════════════════════════════════════════
function ResponsesSection({T, accent, toast$}) {
  const ROWS = [
    {id:1, email:'i.benammar@paytun.is',    nom:'Imen Ben Ammar',   date:'2026-03-11 14:23', company:'PayTunis',   sector:'FinTech',    amount:'500,000 TND',    status:'Active',   score:87},
    {id:2, email:'h.ghariani@dabadoc.tn',   nom:'Hela Ghariani',    date:'2026-03-11 10:15', company:'DabaDoc',    sector:'HealthTech', amount:'1,500,000 TND',  status:'Accepted', score:94},
    {id:3, email:'i.diallo@agrismart.co',   nom:'Ibrahim Diallo',   date:'2026-03-10 16:42', company:'AgriSmart',  sector:'AgriTech',   amount:'200,000 TND',    status:'Pending',  score:71},
    {id:4, email:'s.benali@edulearn.tn',    nom:'Sara Ben Ali',     date:'2026-03-10 09:30', company:'EduLearn',   sector:'EdTech',     amount:'350,000 TND',    status:'Rejected', score:58},
    {id:5, email:'m.khemiri@solartech.io',  nom:'Mohamed Khemiri',  date:'2026-03-09 11:20', company:'SolarTech',  sector:'CleanTech',  amount:'800,000 TND',    status:'Active',   score:82},
  ];
  const STATUS_COLORS = {Active:'#0EA5E9', Accepted:'#10B981', Pending:'#F59E0B', Rejected:'#EF4444'};
  ...
  const sorted = [...ROWS]
    .filter(r=>r.nom.toLowerCase().includes(search.toLowerCase())||r.company.toLowerCase().includes(search.toLowerCase())||r.email.toLowerCase().includes(search.toLowerCase()))
    .sort((a,b)=>{
      const va=a[sortKey], vb=b[sortKey];
      return sortDir==='asc' ? (va>vb?1:-1) : (va<vb?1:-1);
    });

  const toggleSort = k => { if(sortKey===k){setSortDir(d=>d==='asc'?'desc':'asc');}else{setSort(k);setSortDir('asc');} };
  const toggleAll  = () => setSel(sel.length===sorted.length?[]:sorted.map(r=>r.id));
  const toggleRow  = id => setSel(s=>s.includes(id)?s.filter(x=>x!==id):[...s,id]);

  const doExport = () => {
    const data = sel.length ? ROWS.filter(r=>sel.includes(r.id)) : ROWS;
    const csv  = ['Email,Name,Date,Company,Sector,Amount,Status,Score',
      ...data.map(r=>[r.email,r.nom,r.date,r.company,r.sector,r.amount,r.status,r.score].join(','))
    ].join('\n');
    const a = document.createElement('a');
    a.href = URL.createObjectURL(new Blob(['\uFEFF'+csv], {type:'text/csv;charset=utf-8'}));
    a.download = `responses_${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
    toast$(`Exported ${data.length} response${data.length>1?'s':''}`, 'ok');
  };

  const TH = ({k,ch,right=false}) => (
    <th onClick={()=>toggleSort(k)} style={{
      padding:'11px 14px', textAlign:right?'right':'left',
      fontSize:11, fontWeight:700, letterSpacing:'.05em', textTransform:'uppercase',
      color:sortKey===k ? T.acText : T.ink3,
      cursor:'pointer', userSelect:'none', whiteSpace:'nowrap',
      background:T.card2, borderBottom:`2px solid ${T.line}`,
      transition:'color .15s',
    }}>
      {ch} {sortKey===k && (sortDir==='asc'?'↑':'↓')}
    </th>
  );

  const stats = [
    {l:'Total',    v:ROWS.length,                                color:accent},
    {l:'Accepted', v:ROWS.filter(r=>r.status==='Accepted').length, color:'#10B981'},
    {l:'Active',   v:ROWS.filter(r=>r.status==='Active').length,   color:'#0EA5E9'},
    {l:'Pending',  v:ROWS.filter(r=>r.status==='Pending').length,  color:'#F59E0B'},
    {l:'Rejected', v:ROWS.filter(r=>r.status==='Rejected').length, color:'#EF4444'},
  ];
  ...
  return (
    <div style={{display:'flex', flexDirection:'column', gap:14, paddingBottom:48}}>
      ...
      <div style={{display:'grid', gridTemplateColumns:'repeat(5,1fr)', gap:12}}>
        {stats.map(s=>(
          <div key={s.l} style={{
            ...
            <p style={{margin:'0 0 6px', fontSize:11, fontWeight:600, color:T.ink3, textTransform:'uppercase', letterSpacing:'.06em'}}>{s.l}</p>
            <p style={{margin:0, fontSize:26, fontWeight:800, color:s.color, letterSpacing:'-.03em'}}>{s.v}</p>
          </div>
        ))}
      </div>
      ...
      <div style={{
        ...
        <span style={{fontSize:12.5, color:T.ink3, fontWeight:500}}>
          {sorted.length} response{sorted.length!==1?'s':''}
          {sel.length>0 && <span style={{color:T.acText}}> · {sel.length} selected</span>}
        </span>
        <div style={{marginLeft:'auto', display:'flex', gap:8}}>
          <button onClick={toggleAll}
            style={{
              padding:'8px 14px', border:`1px solid ${T.line}`, borderRadius:9,
              background:'transparent', color:T.ink2,
            ...
            {sel.length===sorted.length ? 'Deselect all' : 'Select all'}
          </button>
          <button onClick={doExport}
            style={{
              ...
              display:'flex', alignItems:'center', gap:6,
              boxShadow:`0 2px 8px rgba(${hr(accent)},.25)`,
              transition:'all .16s',
            }}
            onMouseEnter={e=>{e.currentTarget.style.transform='translateY(-1px)'; e.currentTarget.style.boxShadow=`0 4px 14px rgba(${hr(accent)},.38)`;}}
            onMouseLeave={e=>{e.currentTarget.style.transform='none'; e.currentTarget.style.boxShadow=`0 2px 8px rgba(${hr(accent)},.25)`;}}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{display:'block',flexShrink:0}}><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
            Export CSV{sel.length>0&&` (${sel.length})`}
          </button>
        </div>
      </div>
      ...
      <div style={{
        ...
        <table style={{width:'100%', borderCollapse:'collapse', minWidth:900}}>
          <thead>
            <tr>
              <th style={{padding:'11px 14px', background:T.card2, borderBottom:`2px solid ${T.line}`, width:42, textAlign:'center'}}>
                <input type="checkbox" checked={sel.length===sorted.length&&sorted.length>0}
                  onChange={toggleAll}
                  style={{accentColor:accent, width:15, height:15, cursor:'pointer'}}/>
              </th>
              <TH k="nom"     ch="Name"/>
              <TH k="email"   ch="Email"/>
              <TH k="company" ch="Company"/>
              <TH k="sector"  ch="Sector"/>
              <TH k="amount"  ch="Amount" right/>
              <TH k="score"   ch="Score"/>
              <TH k="status"  ch="Status"/>
              <TH k="date"    ch="Submitted"/>
            </tr>
          </thead>
          <tbody>
            {sorted.map((r,idx)=>{
              ...
              return (
                <tr key={r.id}
                  style={{
                    background:on?T.acBg:'transparent',
                    borderBottom:idx<sorted.length-1?`1px solid ${T.line}`:'none',
                    transition:'background .13s',
                  }}
                  onMouseEnter={e=>{if(!on)e.currentTarget.style.background=T.hover;}}
                  onMouseLeave={e=>{if(!on)e.currentTarget.style.background='transparent';}}>
                  <td style={{padding:'13px 14px', textAlign:'center'}}>
                    <input type="checkbox" checked={on} onChange={()=>toggleRow(r.id)}
                      style={{accentColor:accent, width:15, height:15, cursor:'pointer'}}/>
                  </td>
                  <td style={{padding:'13px 14px'}}>
                    <div style={{display:'flex', alignItems:'center', gap:9}}>
                      <div style={{
                        ...
                        display:'flex', alignItems:'center', justifyContent:'center',
                        color:'#fff', fontSize:10, fontWeight:800, letterSpacing:'-.02em',
                      }}>{r.nom.split(' ').map(n=>n[0]).join('')}</div>
                      <span style={{fontSize:13, fontWeight:600, color:T.ink, whiteSpace:'nowrap'}}>{r.nom}</span>
                    </div>
                  </td>
                  <td style={{padding:'13px 14px', fontSize:12.5, color:T.ink2}}>{r.email}</td>
                  <td style={{padding:'13px 14px', fontSize:13, fontWeight:600, color:T.ink}}>{r.company}</td>
                  <td style={{padding:'13px 14px'}}>
                    <Chip color={(() => {const COLORS={FinTech:'#0EA5E9',HealthTech:'#10B981',AgriTech:'#84CC16',EdTech:'#F59E0B',CleanTech:'#06B6D4'}; return COLORS[r.sector]||'#94a3b8';})()}>
                      {r.sector}
                    </Chip>
                  </td>
                  <td style={{padding:'13px 14px', fontSize:12.5, fontFamily:'monospace', color:T.ink2, textAlign:'right', whiteSpace:'nowrap'}}>{r.amount}</td>
                  <td style={{padding:'13px 14px'}}>
                    <div style={{display:'flex', alignItems:'center', gap:7}}>
                      <div style={{flex:1, height:4, background:T.line, borderRadius:99, overflow:'hidden', maxWidth:60}}>
                        <div style={{height:'100%', width:`${r.score}%`, background:r.score>=80?'#10B981':r.score>=60?'#F59E0B':'#EF4444', borderRadius:99, transition:'width .5s'}}/>
                      </div>
                      <span style={{fontSize:12, fontWeight:700, color:r.score>=80?'#10B981':r.score>=60?'#F59E0B':'#EF4444'}}>{r.score}</span>
                    </div>
                  </td>
                  <td style={{padding:'13px 14px'}}>
                    <span style={{
                  ...
                  <td style={{padding:'13px 14px', fontSize:12, color:T.ink3, whiteSpace:'nowrap'}}>{r.date}</td>
                </tr>
              );
            })}
          </tbody>
        ...
        {sorted.length===0 && (
          <div style={{padding:'48px 24px', textAlign:'center', color:T.ink3, fontSize:13}}>
            No responses match your search.
          </div>
        )}
      </div>

      <p style={{margin:0, fontSize:11.5, color:T.ink3, textAlign:'right'}}>
        Showing {sorted.length} of {ROWS.length} responses
...
function SettingsSection({accent,T}) {
  ...
  const Toggle = ({on, set, label, desc}) => (
    <div style={{
      display:'flex', alignItems:'center', justifyContent:'space-between',
      padding:'15px 0', borderBottom:`1px solid ${T.line}`, gap:20,
    }}>
      <div>
      ...
      <button onClick={()=>set(!on)}
        style={{
          position:'relative', width:40, height:22, borderRadius:100,
          border:'none', cursor:'pointer', flexShrink:0,
          background:on?accent:T.line2, transition:'background .2s',
        }}>
        <div style={{
  ...
  const fld = {
    border:`1px solid ${T.line}`, borderRadius:9, padding:'9px 13px',
    fontSize:13, background:T.card2, color:T.ink,
    ...
    <div style={{maxWidth:600, display:'flex', flexDirection:'column', gap:14, paddingBottom:48}}>
      ...
      <Group title="Presentation">
        ...
        <div style={{padding:'16px 0'}}>
          ...
          <textarea value={confirmMsg} onChange={e=>setCM(e.target.value)} rows={2}
            placeholder="Thank you for your response!"
            style={{...fld, width:'100%', resize:'none', display:'block'}}
            onFocus={e=>e.target.style.borderColor=accent}
            onBlur={e=>e.target.style.borderColor=T.line}
          />
        </div>
        ...
        <div style={{padding:'16px 0', display:'flex', flexDirection:'column', gap:16}}>
          ...
          <div>
            ...
            <div style={{display:'flex', alignItems:'center', gap:10}}>
              <input type="number" min={1} max={30} value={reminder}
                onChange={e=>setRM(+e.target.value)}
                style={{...fld, width:64, textAlign:'center', fontSize:14}}
                onFocus={e=>e.target.style.borderColor=accent}
                onBlur={e=>e.target.style.borderColor=T.line}
              />
